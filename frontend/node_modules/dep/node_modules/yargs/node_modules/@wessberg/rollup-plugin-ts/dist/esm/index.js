    ...
    switch (declaration === null || declaration === void 0 ? void 0 : declaration.kind) {
        ...
        case typescript.SyntaxKind.ClassExpression: {
            return [
                ...
                preserveParents(compatFactory.createImportEqualsDeclaration(undefined, undefined, compatFactory.createIdentifier(name), compatFactory.createQualifiedName(compatFactory.createIdentifier(moduleBinding), compatFactory.createIdentifier(propertyName))), { typescript })
            ];
        }
...
function visitClassDeclaration$4({ node, continuation, compatFactory }) {
...
function visitClassExpression$4({ node, continuation, compatFactory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitFunctionDeclaration$4({ node, continuation, compatFactory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitFunctionExpression$4({ node, continuation, compatFactory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitEnumDeclaration$4({ node, continuation, compatFactory }) {
...
function visitTypeAliasDeclaration$4({ node, continuation, compatFactory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitVariableStatement$4({ node, continuation, compatFactory }) {
...
function visitImportDeclaration$3({ node, continuation, compatFactory }) {
    if (node.importClause == null)
        return undefined;
...
function visitImportSpecifier$1({ node, continuation, compatFactory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitImportClause$1({ node, continuation, compatFactory }) {
...
function visitNamespaceImport$1({ node, continuation, compatFactory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function treeShaker(options) {
    ...
    // Prepare some VisitorOptions
    const visitorOptions = Object.assign(Object.assign({}, options), { treeshakenCommentRanges: [], isReferenced: (node) => isReferenced(Object.assign(Object.assign({}, visitorOptions), { node })), continuation: (node) => visitNode$9(Object.assign(Object.assign({}, visitorOptions), { node })) });
...
function toExportDeclarationTransformer(options) {
    ...
    const nodePlacementQueue = getNodePlacementQueue({ typescript });
    // Prepare some VisitorOptions
    const visitorOptions = Object.assign(Object.assign(Object.assign({}, options), nodePlacementQueue), { childContinuation: (node) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$a(Object.assign(Object.assign({}, visitorOptions), { node: nextNode }))), context), continuation: (node) => nodePlacementQueue.wrapVisitResult(visitNode$a(Object.assign(Object.assign({}, visitorOptions), { node }))) });
    let result = typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context);
    // There may be prepended or appended nodes that hasn't been added yet. Do so!
    const [missingPrependNodes, missingAppendNodes] = nodePlacementQueue.flush();
    if (missingPrependNodes.length > 0 || missingAppendNodes.length > 0) {
